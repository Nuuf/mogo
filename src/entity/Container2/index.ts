...
export default class Container2 extends Core2 {
  ...
  GetChildClosestTo<T extends Container2>(p: Point2, filter: () => boolean): T | null | undefined {
    const { children } = this;
    let [child] = children;
    ...
    if (children.length !== 0) {
      for (var i = 1; i < children.length; child = children[++i]) {
        if (filter !== undefined) {
          if (filter(child) === false) continue;
        }
  ...
  GetChildFurthestFrom<T extends Container2>(p: Point2, filter: () => boolean): T | null | undefined {
    const { children } = this;
    let [child] = children;
    let distance = Infinity;
    let tempDistance = 0;
